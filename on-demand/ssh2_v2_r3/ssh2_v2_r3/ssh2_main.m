...
if (ssh2_struct.sftp > 0) 
    ...
    try
        if (getFile > 0)
            fprintf('JAVA/MATLAB incompatibility\nCannot use SFTP to retrieve files, please use SCP instead!\n');
            ...
            for getIndex = 1:size(strArrayGet,2)
                ...
                count=0; 
                buf = javaArray('java.lang.Byte', 1024);
                %buf = uint8(zeros(1,1024));
                bufsize = size(buf,1);
                local_fileid = fopen(localFilePath,'w');
                %[buf, bufsize] = fread(local_fileid, 1024);
                readcnt = 0; 

                try
                    while(readcnt~=-1)
                        readcnt = sftp1.read(remotef,uint32(count),buf,uint16(0),uint16(bufsize));
                        count=readcnt+count;
                        fwrite(local_fileid, buf);
                    end   
                catch 
                     error(['Error: SFTP could not write to the file: %s, '...
                        ' on local machine!'],localFilePath);
                end
