...
for i = 1:numMovies
    ...
    if str2num(tmpruntime{i}(1:index-2)) > 30
        ...
        try
            img = GetMovieImageFromTMDB(title{j},year{j});
            imwrite(img,fullfile('MoviePics',[title{j} '.jpg']),'jpg');
        end
    end
end
numMovies = j;
...
for i = 1:numMovies
    fprintf(fid,'\t%s\n','<tr>');
    fprintf(fid,'\t%s\n',['<td><span class="dropt"><a href="' trailerlink{i} '" target="_blank"><B><font size=3><center>' title{i} '</center></font></B></a> <span><img src="MoviePics/' title{i} '.jpg"/></span></span>']);
    ...
    if status == 1
        ...
        while i2 < length(info2)
            ...
            if status == 1
                ...
                try
                    img = GetMovieImageFromTMDB(title{numMovies});
                    imwrite(img,fullfile('MoviePics',[title{numMovies} '.jpg']),'jpg');
                end
                break;
...
while i < length(info)
    ...
    if status == 1
        ...
        % save movie image
        try
            img = GetMovieImageFromTMDB(title{numMovies},year{numMovies});
            imwrite(img,fullfile('MoviePics',[title{numMovies} '.jpg']),'jpg');
        end
    end
end
...
HOSTNAME = '24.191.43.45';
USERNAME = 'peter';
PASSWORD = '';
ssh2_conn = ssh2_config(HOSTNAME, USERNAME, PASSWORD, 22);
...
list = dir('MoviePics');
for i = 3:length(list)
    ssh2_conn = scp_simple_put(HOSTNAME,USERNAME,PASSWORD,list(i).name,'/var/www/MoviePics','/Users/pkk23/Desktop/GetMovieInfo/Movies On Demand/MoviePics');
    disp([num2str(i) ' out of ' num2str(length(list))]);
end  
ssh2_conn = ssh2_command(ssh2_conn, 'sudo -u peter| chmod 775 /var/www/MoviePics/*');
